...
,(VALORORIGINAL+ACRE_ACORDO-DESCONTOS_GERAL) AS VALOR_TOTAL
...
(SELECT 
 *
,dbo.fn_numero_por_extenso(VALORTOTAL2) AS VALOREXTENSO1

 FROM 
      (
       SELECT  * 
	   ,(SELECT MAX(QTD_ACORDOS) FROM TAB_LIG  WHERE IDACORDO=@ACC) AS MAX_ACC
	   ,(SELECT CONVERT(VARCHAR,CONVERT(DECIMAL(18,2),SUM(VALOR_TOTAL)))  FROM TAB_LIG  WHERE IDACORDO=@ACC AND CLASSACORDO=2 AND CONVERT(VARCHAR,(CASE WHEN QTD_ACORDOS=1 THEN GETDATE() ELSE DATABAIXA END),103)<=CONVERT(VARCHAR,(CASE WHEN QTD_ACORDOS=1 THEN GETDATE() ELSE DATAEMISSAO END),103)) AS VALORTOTAL2 /*AND CONVERT(VARCHAR,DATABAIXA,103)<=CONVERT(VARCHAR,RECCREATEDON,103)*/
	   ...
	   GROUP BY IDLAN,ACORDO_ANT,INDEXADOR,DATABAIXA,DATAEMISSAO,FACORDODATA,QTD_ACORDOS,CODCOLIGADA,SERVICO1,VENCIMENTO,CODFILIAL,IDACORDO,NUMERODOCUMENTO,CLASFLAN,CLASSACORDO,STATUSLAN,MES_COMP,CODCFO,IND,IDPARCELA,TIPOPARCELA,PARCELA,CODESP,VALORORIGINAL,VALORBAIXADO,VALORORIGINALBX,VALORDESCONTOBX,JUROSBX,MULTABX,DESC_ACORDO,ACRE_ACORDO,HDESCONTOACORDO,CONTA,RA,BREC,CODAA,VERIFICADOR,DESCONTOS_GERAL,ACORDOSS,ACORD_LANS,SERVICO,VALOR_TOTAL,MAX_ACC,VALORTOTAL2,HACRESCIMOACORDO
	   HAVING  CONVERT(VARCHAR,(CASE WHEN QTD_ACORDOS=1 THEN GETDATE() ELSE DATABAIXA END),103)<=CONVERT(VARCHAR,(CASE WHEN QTD_ACORDOS=1 THEN GETDATE() ELSE DATAEMISSAO END),103) AND CLASFLAN=15 --AND (CASE STATUSLAN='PAGO' THEN CONVERT(VARCHAR,DATABAIXA,103)<=CONVERT(VARCHAR,DATAEMISSAO,103) END )
	   ) 
