	...
	SALUNO (NOLOCK) 
		...
		AND 
			SMATRICPL.RA = SALUNO.RA
			 ...
			 LEFT JOIN SMATRICULA (NOLOCK) ON SMATRICULA.RA = SMATRICPL.RA AND SMATRICULA.IDPERLET = SMATRICPL.IDPERLET AND SMATRICULA.IDHABILITACAOFILIAL = SMATRICPL.IDHABILITACAOFILIAL

			   JOIN STURMADISC (NOLOCK) ON STURMADISC.IDHABILITACAOFILIAL = SMATRICULA.IDHABILITACAOFILIAL AND STURMADISC.IDTURMADISC = SMATRICULA.IDTURMADISC

			   LEFT JOIN SDISCIPLINA (NOLOCK) ON SDISCIPLINA.CODDISC = STURMADISC.CODDISC
...
/*Abaixo, alterado de 'SPLETIVO.CODPERLET LIKE' para 'SPLETIVO.CODPERLET =' por motivo de performance. Augusto Ambrozio*/
			WHERE 
				SPLETIVO.CODPERLET = '2019/1'--:COD_PERLET
			AND 
				SMATRICPL.CODFILIAL = 18--:COD_FILIAL
			AND 
				SPLETIVO.CODTIPOCURSO = 4--:TIPO_CURSO

				/* --  AND SMATRICPL.RA='00019513' AND CODDISC = 'PSICOLORG'*/
