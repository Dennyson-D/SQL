...
WHERE D.TIPOCONTRATO='A' AND E.DTVENCIMENTO >= '01/01/2019' --/*B.CODPERLET = '2019/1'  AND D.TIPOCONTRATO='P' AND L.MESDECOMPETENCIA BETWEEN '01/01/2019' AND '30/06/2019' AND L.STATUSLAN NOT IN (2,3) AND*/ AND L.CODCFO <> '006747'
...
,ISNULL(( CASE WHEN STATUS_CONTRATO = 'ATIVO'  THEN ( CASE WHEN ACORDO  LIKE '%MENSALIDADE%' THEN ISNULL((TABA.VALORORIGINAL - TABA.VALORBOLSA - TABA.BOLSAS_INCONDICIONAIS),TABA.VAL_PARCELA)  ELSE CASE WHEN ACORDO LIKE '%MENSALIDADE%' AND STATUSLAN = 'ABERTO' THEN VAL_PARCELA ELSE VLR_SBLAN END END ) ELSE 0 END),0) AS FATURAMENTO
 ...
 ------------------------FATURAMENTO------------------------------------------------------------------------------------------------------------------------
 /*
 SELECT 'PÓS' AS TIPO_CURSO,* FROM TABC -- WHERE/* SITUACAO LIKE 'MAT%' AND*/ RA= '00005647'--'00024826' --'00024488' --'00024826'--'00005647' --AND IDPERLET =277 


  
 UNION ALL

 SELECT 'PÓS' AS TIPO_CURSO,* FROM TABB  -- WHERE /*SITUACAO LIKE 'MAT%' AND*/ RA= '00005647'--'00024826' --'00024488' --'00024826'--'00005647' --AND IDPERLET =277 
 

 

ORDER BY TABC.CURSO, TABC.RA,PARCELA, ACORDO
 
 */
 ------------------------CONTAGEM------------------------------------------------------------------------------------------------------------------------
 
 SELECT 'PÓS' AS TIPO_CURSO, CURSO, MESDECOMPETENCIA AS MÊS_COMPETÊNCIA,RA,ANO --,CONTA AS QTDE_ALUNOS
  FROM TABCONTA WHERE STATUS_CONTRATO='ATIVO' ORDER BY CURSO,MES
  
  
  ------------------------BOLSAS------------------------------------------------------------------------------------------------------------------------
 --SELECT * FROM TABBOLSAS ORDER BY CURSO,ACORDO,MESDECOMPETENCIA,RA
