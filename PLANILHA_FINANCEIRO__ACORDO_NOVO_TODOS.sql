...
FROM SMATRICPL A (NOLOCK)
 ...
 JOIN SCURSO J (NOLOCK) ON J.CODCURSO = I.CODCURSO AND J.CODTIPOCURSO IN (4,6,11)
...
WHERE D.TIPOCONTRATO = 'A' AND E.DTVENCIMENTO >= '01/01/2019' AND L.STATUSLAN NOT IN (2)--AND L.IDLAN=1533062 --/*B.CODPERLET = '2019/1' L.STATUSLAN NOT IN (2,3) AND*/ AND L.CODCFO <> '006747'
 ...
 ,D.STATUS,D.DTCANCELAMENTO,J.CODTIPOCURSO
...
ISNULL(TABA.ACORDO1,'')AS ACORDO1,
TABA.IDPARCELA,
TABA.CODFILIAL,
...
,ISNULL(CASE WHEN ACORDO LIKE '%MENSALIDADE%' AND STATUSLAN <> 'ACORDO' THEN VALORBAIXADO ELSE (CASE WHEN TABA.STATUSLAN ='ACORDO' THEN 0 ELSE 0 END )END,0) AS VALORBX_SBLAN
...
,ISNULL(( CASE WHEN STATUS_CONTRATO = 'ATIVO'  THEN ( CASE WHEN ACORDO  LIKE '%MENSALIDADE%' THEN ISNULL((TABA.VALORORIGINAL - (CASE WHEN STATUSLAN = 'ACORDO' THEN 0 ELSE VALOROP2 END )/*VLR_SBLAN*/ - TABA.BOLSAS_INCONDICIONAIS),TABA.VAL_PARCELA)  ELSE CASE WHEN ACORDO LIKE '%MENSALIDADE%' AND STATUSLAN = 'ABERTO' THEN VAL_PARCELA ELSE CASE WHEN STATUSLAN ='ABERTO' OR STATUSLAN ='ACORDO'  THEN 0 ELSE VLR_SBLAN END END END ) ELSE CASE WHEN ACORDO LIKE '%BOLSA%' THEN VLR_SBLAN ELSE VALORORIGINAL END END),0) AS FATURAMENTO
,CODTIPOCURSO
    -- ,VALOROP2
 FROM TABA
 ...
 ------------------------FATURAMENTO------------------------------------------------------------------------------------------------------------------------
 /*
 SELECT CASE CODTIPOCURSO WHEN 4 THEN 'GRAD' WHEN 6 THEN 'POS' WHEN 11 THEN 'EAD' END AS TIPO_CURSO,* FROM TABC  --WHERE STATUS_CONTRATO ='CANCELADO' --IDLAN =1555397 --IN  (1532111,1529713)  --  1531608
 -- 1529713
...
--WHERE IDLAN=1565444--1540448
 ...
 SELECT CASE CODTIPOCURSO WHEN 4 THEN 'GRAD' WHEN 6 THEN 'POS' WHEN 11 THEN 'EAD' END AS TIPO_CURSO,* FROM TABB  --WHERE STATUS_CONTRATO ='CANCELADO'--IDLAN =1555397 --IN (1532111,1529713) --1531608
 --1529713
...
--WHERE IDLAN=1565444--1540448
 ...
 ORDER BY TABC.CODTIPOCURSO, TABC.CURSO, TABC.RA, TABC.IDLAN, ACORDO
 */
